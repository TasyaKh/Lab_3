...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            //comboBoxSymbol1.SelectedIndex = comboBoxSymbol2.SelectedIndex = 0;
            //comboBoxSymbol3.SelectedIndex = 0;
        ...
        private void comboBoxColor_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (comboBoxDates1.SelectedIndex == 0)                                       //Если выбранные единицы измерения градусы
            {
                comboBoxDates1.SelectedIndex = 0;                                        //Обнулить бокс с данными цвета
                textBoxColor1.Text = hsv.typeColorByIndex(comboBoxColor.SelectedIndex);  //Приравниваем к соответствующему цвету
            }
            else comboBoxColor.SelectedIndex = -1;
        }
        ...
        private void textBox_Leave(object sender, EventArgs e)
        {
            ...
            if (!check.diapasonValuesIsValid(txt.Text, typeDate, keepTagElem))
            {
                MessageBox.Show(check.getMessageError());
                txt.Text = "0";
 
            }

            //if (keepTagElem == 3)         //Если выбрано поле насыщенности
            //    textBoxS.Text = txt.Text; //Приравниваем к боксу насыщенности значение поля насыщенности
            //else if (keepTagElem == 5)    //Если выбрано поле яркости
            //    textBoxV.Text = txt.Text; //Аналогично действиям сверху                              
        }
        private void textBoxColor1_Leave(object sender, EventArgs e)
        {
            if (check.diapasonValuesIsValid(textBoxColor1.Text, comboBoxDates1.Text, 1))
            {                                                                   //Если данные цвета действительны
                ...
                {                                                              //Если цвет не входит в диапазон
                    ...
                    {                                                          //Если тип данных градусы
                        ...
                        textBoxColor1.Text = hsv.typeColorByIndex(comboBoxColor.SelectedIndex); //Сбросить поле до нужного цвета
                    }  
        ...
        private void comboBoxDates2_SelectedIndexChanged(object sender, EventArgs e)
        {
          ...
          // textBoxS.Text = textBoxSaturation1.Text; //приравниваем доп textBox к 0
        }
        ...
        {
            bool checkFirstField = check.diapasonValuesIsValid(textBoxColor1.Text, comboBoxDates1.Text, 1); //П
            bool checkSecondField = check.diapasonValuesIsValid(textBoxColor2.Text, comboBoxDates1.Text, 2);
            ...
            else    //Если данные корректны
            {
                ...
                else if (check.diapasonValuesIsValid(Convert.ToString(endColor), comboBoxDates1.Text, 1))
        ...
        private void comboBoxSymbol2And3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                checkFirstField = check.diapasonValuesIsValid(field1.Text, fieldCmbBox.Text,
                    Convert.ToInt32(field1.Tag));                         //Проверить корректность анных поля 1
                checkSecondField = check.diapasonValuesIsValid(field2.Text, fieldCmbBox.Text, 
                ...
                else                                                       //Иначе сложить или вычесть поле1 со 2ым и 
                {
                    ...
                    if (check.diapasonValuesIsValid(endValue, fieldCmbBox.Text,
                    ...
                    else MessageBox.Show(check.getMessageError());         //Иначе сообшить об ошибке
        ...
        {
            bool H = check.diapasonValuesIsValid(textBoxColor1.Text,comboBoxDates1.Text,1);
            bool S = check.diapasonValuesIsValid(textBoxSaturation1.Text, comboBoxDates2.Text, 3);
            bool B = check.diapasonValuesIsValid(textBoxBright1.Text, comboBoxDates3.Text, 5);
